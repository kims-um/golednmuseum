...
app.post('/graded-cards', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('graded_cards')
    .upsert({ cert_number, card_name, grade, year, brand, variety, image_url, grader: grader || 'PSA', avg_cost: avg_cost || 0 })
    .select();
  if (error) return res.status(500).json({ error: error.message });

  await supabase.from('transactions').insert({
    type: 'in', product_type: 'graded_card',
    name: card_name, qty: 1, price: avg_cost, total: avg_cost, cost: avg_cost, pay_method: '-'
  });
...
app.post('/products', async (req, res) => {
  ...
  if (existing) {
    ...
    const { data, error } = await supabase.from('products')
      .update({ qty: newQty, avg_cost: newCost }).eq('barcode', barcode).select();
    if (error) return res.status(500).json({ error: error.message });
    await supabase.from('transactions').insert({
      type: 'in', product_type: 'product', product_id: existing.id,
      name, qty, price: avg_cost, total: avg_cost * qty, cost: avg_cost, pay_method: '-'
    });
    return res.json(data);
  }

  const { data, error } = await supabase.from('products')
    .insert({ barcode, name, type, country, packs, sell_price, avg_cost: avg_cost || 0, qty: qty || 1 }).select();
  if (error) return res.status(500).json({ error: error.message });

  await supabase.from('transactions').insert({
    type: 'in', product_type: 'product', product_id: data[0].id,
    name, qty, price: avg_cost, total: avg_cost * qty, cost: avg_cost, pay_method: '-'
  });
...
app.post('/sell', async (req, res) => {
  ...
  for (const item of items) {
    ...
    await supabase.from('transactions').insert({
      type: 'out', product_type: item.product_type, product_id: item.product_id,
      name: item.name, qty: item.qty, price: item.price, total: item.price * item.qty,
      cost: item.cost, pay_method
    });
  }
